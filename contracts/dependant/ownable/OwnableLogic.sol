...
contract OwnableLogic is OwnableLayout, OwnableInterface {
    ...
    modifier onlyAuth() {
        require(isAuth(), "Ownable: caller is un-authed");
        _;
    }

    function isAuth() internal view returns (bool){
        return owner() == msg.sender || _associatedOperators[msg.sender];
    }

    modifier onlyAuthTrusted{
        require(_onlyAuthTrusted(), "Ownable: onlyAuthTrusted");
        _;
    }

    function _onlyAuthTrusted() virtual view internal returns (bool){
        return isAuth();
    }

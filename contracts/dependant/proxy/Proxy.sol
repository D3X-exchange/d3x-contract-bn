    ...
    function process() internal outOfService {
        ...
        */
        ...
        if (sysGetRevertMessage() == 1) {
            revert(string(abi.encodePacked(sysPrintAddressToHex(address(this)), ", function selector not found : ", sysPrintBytes4ToHex(msg.sig), ", caller : ", sysPrintAddressToHex(msg.sender))));
        } else {
            revert();
        }
